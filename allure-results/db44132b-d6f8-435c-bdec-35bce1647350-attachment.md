...
```ts
  ...
  94  |     await this.appointmentDate.fill('');
  95  |     await this.appointmentDate.fill(details.appointmentDate);
  96  |     await this.appoitmentDateClsBtn.click();
  97  |     const availableSlots = this.slot.locator('option:not([value=""])');
  98  |     await expect.poll(async () => availableSlots.count()).toBeGreaterThan(0);
  99  |     const firstAvailableSlot = await availableSlots.first().getAttribute('value');
  100 |     if (!firstAvailableSlot) {
  101 |       throw new Error('No appointment slot was available for the selected date');
  102 |     }
  103 |     await this.slot.selectOption({ value: firstAvailableSlot });
  104 |     await expect(this.slot).toHaveValue(firstAvailableSlot);
  ...
  112 |     await this.appoitmentSaveBtn.click();
  113 |     await expect(this.successMessage).toBeVisible();
> 114 |     await expect(this.successMessage).toHaveText('Record Saved Successfully');
      |                                       ^ Error: expect(locator).toHaveText(expected) failed
  115 |   }
  116 | 
  117 |   async deleteAppointment(patientName: string, patientId: string): Promise<void> {
  118 |     const appointmentRow = this.appointmentRows
  119 |       .filter({ hasText: `${patientName} (${patientId})` })
  120 |       .first();
  121 | 
  122 |     await expect(appointmentRow).toBeVisible();
  123 |     await expect(appointmentRow.locator('td').first()).toContainText(`${patientName} (${patientId})`);
  124 | 
  125 |     const appointmentActions = appointmentRow.locator('.rowoptionview');
  126 |     await appointmentActions.hover();
  127 |     await appointmentActions.locator('a[aria-label="Show"]').click();
  128 | 
  129 |     await expect(this.deleteButton).toBeVisible();
  130 |     this.page.once('dialog', dialog => dialog.accept());
  131 |     await this.deleteButton.click();
  132 | 
  133 |     await expect(this.successMessage).toBeVisible();
  134 |     await expect(this.successMessage).toHaveText('Record Deleted Successfully');
  135 |   }
  136 | }
  137 | 
```